...
import '../ts-checkbox/ts-checkbox';
import './ts-table-header'
import '../ts-pagination/ts-pagination';
import {
    findParentComponent,
    parseSlotToVModel,
...
avalon.component('ts-table', {
    soleSlot: 'header',
    template: require('./ts-table.html'),
    defaults: {
        header: '',
        columns: [],
        data: [],
...
function getColumnConfig(descriptor, level = 1) {
    return descriptor.reduce((acc, column) => {
        if (column.is != 'ts-table-header') return acc;
        if (column.props.type == 'selection') {
            return acc;
        }
        ...
        inlineTemplate = inlineTemplate.replace(/(ts-|:)click="handle\(([^"]*)\)"/g, ($0, $1, $2, $3) => {
